...
public:
     ...
     */
    //add throw tree exception thing where needed
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
        Node<K, V> *y;
        //go through violations to break out of the otherwise infinite recusion
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        if (root_ == nullptr || (node->right == nullptr && node->left == nullptr)) {
            return 0;
        }
        else if (node->right == nullptr) {
            return 1 + internal_node_count(node->left);
        }
        else if (node->left == nullptr) {
            return 1 + internal_node_count(node->right);
        }
        else {
     ...
     */
    int diameterHelper(Node<K, V> *node) const {
            if(root_ == nullptr) {
                return 0;
            }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node->left == nullptr && node->right == nullptr){
            ...
            return level + 1 + sum_null_levels(node->right, level + 1);
        }
        else if (node ->right == nullptr) {
            return level + 1 + sum_null_levels(node->left, level + 1);
        }
        else{
